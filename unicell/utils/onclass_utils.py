...
def read_data(adata, feature_file, cell_ontology_ids, exclude_non_leaf_ontology = False, ct_mapping_key = {}, tissue_key = None, seed = 1, filter_key = None, AnnData_label_key=None, nlp_mapping = True, nlp_mapping_cutoff = 0.8, co2emb = None, label_file=None, cl_obo_file = None, cell_ontology_file = None):
	np.random.seed(seed)
	x = read_h5ad(feature_file) if feature_file is not None else adata
	ncell = np.shape(x.X)[0]
	dataset = x.X
