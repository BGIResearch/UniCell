...
GENE_MEDIAN_FILE = os.path.join(curr_dir, "gene_median_dict.json")
TOKEN_DICTIONARY_FILE = os.path.join(curr_dir, "gene_vocab.json")
...
class TranscriptomeTokenizer:
    ...
    def tokenize_anndata(self, adata, target_sum=10_000, chunk_size=512):
        ...
        for i in range(0, len(filter_pass_loc), chunk_size):
            ...
            X_norm = adata[idx, coding_miRNA_loc].X
            ...
            if self.custom_attr_name_dict is not None:
                for k in file_cell_metadata.keys():
                    file_cell_metadata[k] += adata[idx].obs[self.custom_attr_name_dict[k]].tolist()
            else:
